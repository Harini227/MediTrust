...
async function extract(filePath) {
  // Attempt the configured provider but fall back to the dummy extractor
  // if the configured provider fails (missing API key, network error, etc.).
  try {
    if (config.providers.ocr === 'groq') {
      return await groqVisionProvider.extract(filePath);
    }
    if (config.providers.ocr === 'azure') {
      return await azureProvider.extract(filePath);
    }
    // Default fallback
    return await dummyExtract(filePath);
  } catch (err) {
    logger.error('OCR provider failed, falling back to dummy OCR: ' + (err.message || err));
    // Use dummyExtract so the rest of the workflow still has a usable OCR shape
    return await dummyExtract(filePath);
  }
}
